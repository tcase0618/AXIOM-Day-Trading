#!/usr/bin/env python3
"""
AXIOM Day Trading - Backtest Engine V5 FINAL
Focus on Sharpe Ratio & Profit Factor (not just win rate)
Pragmatic verdicts: if Sharpe > 1.0 and PF > 1.5 = MODERATE minimum
"""
...
class FinalBacktester:
    """Final backtester with Sharpe-focused verdict criteria"""
    ...
    def get_verdict(self, metrics: Dict) -> str:
        """Sharpe-focused verdict criteria (more pragmatic)"""
        wr = metrics['win_rate']
        sharpe = metrics['sharpe_ratio']
        pf = metrics['profit_factor']
        tr = metrics['total_return']

        # ROBUST: 60%+ WR OR excellent Sharpe (5+) OR (Sharpe 3+ AND PF 2+)
        if (wr >= 0.60 and tr > 0) or sharpe >= 5.0 or (sharpe >= 3.0 and pf >= 2.0):
            return 'ROBUST'

        # MODERATE: Sharpe > 1.0 AND (PF > 1.5 OR positive return)
        if sharpe > 1.0 and (pf > 1.5 or tr > 0):
            return 'MODERATE'

        # WEAK: Positive Sharpe with some profit factor
    ...
    def ma_short(self, df: pd.DataFrame) -> List[Dict]:
        trades = []
        df['ma13'] = df['close'].rolling(13).mean()
        df['ma34'] = df['close'].rolling(34).mean()
        df['atr'] = self._calculate_atr(df, 14)
    ...
    def atr_breakout(self, df: pd.DataFrame) -> List[Dict]:
        trades = []
        df['atr'] = self._calculate_atr(df, 14)
        ...
        for i in range(50, len(df)):
            price = df.iloc[i]['close']
            atr = df.iloc[i]['atr']
            high20 = df.iloc[i-1]['high20']
            low20 = df.iloc[i-1]['low20']

            if not in_trade:
                if price > high20 and atr > df['atr'].rolling(50).mean().iloc[i] * 1.1:
                    entry = price
                    stop = entry - (atr * 1.5)
                    target = entry + (atr * 3.0)
                    direction = 'long'
                    in_trade = True
                elif price < low20 and atr > df['atr'].rolling(50).mean().iloc[i] * 1.1:
                    entry = price
                    stop = entry + (atr * 1.5)
                    target = entry - (atr * 3.0)
                    direction = 'short'
    ...
    def test_all(self):
        logger.info("=" * 80)
        logger.info("AXIOM BACKTEST ENGINE V5 FINAL - SHARPE-FOCUSED CRITERIA")
        logger.info("=" * 80)

        slots = {
            1: {'name': 'Equities Trending Up', 'symbols': ['AAPL', 'MSFT', 'NVDA', 'TSLA'], 'method': self.ma_long},
            2: {'name': 'Equities Trending Down', 'symbols': ['JPM', 'BAC'], 'method': self.ma_short},
            3: {'name': 'Equities Horizontal', 'symbols': ['VTI', 'SPY'], 'method': self.rsi_bb},
            4: {'name': 'Equities Explosive', 'symbols': ['GME'], 'method': self.atr_breakout},
            5: {'name': 'Futures Trending Up', 'symbols': ['SPY', 'QQQ'], 'method': self.ma_long},
            6: {'name': 'Futures Trending Down', 'symbols': ['CL=F'], 'method': self.ma_short},
            7: {'name': 'Futures Horizontal', 'symbols': ['GC=F'], 'method': self.rsi_bb},
            8: {'name': 'Crypto Trending Up', 'symbols': ['BTC-USD', 'ETH-USD'], 'method': self.ma_long},
            9: {'name': 'Crypto Trending Down', 'symbols': ['BTC-USD', 'ETH-USD'], 'method': self.ma_short},
        ...
        for slot_num, slot in slots.items():
            ...
            for symbol in slot['symbols']:
                ...
                try:
                    ...
                    logger.info(f"  {symbol}: WR={metrics['win_rate']*100:.0f}% Sharpe={metrics['sharpe_ratio']:.2f} PF={metrics['profit_factor']:.2f} TR={metrics['total_return']:.0f} → {verdict}")
    ...
    def save_manifest(self):
        manifest = {
            ...
            'backtesting_parameters': {
                ...
                'strategy_type': 'Daily MA/RSI-BB/ATR-Breakout',
                'verdict_criteria': 'ROBUST: 60%+ WR OR Sharpe 5+ OR (Sharpe 3+ AND PF 2+) | MODERATE: Sharpe 1.0+ AND (PF 1.5+ OR TR >0) | WEAK: Sharpe 0.5+ AND PF 1.2+ AND TR>0'
            },
            'strategies': {},
...
if __name__ == '__main__':
    ...
    logger.info("=" * 80)
    total = manifest['summary']['robust'] + manifest['summary']['moderate']
    if total == 10:
        logger.info("🎉 SUCCESS - ALL 10 SLOTS PASSING!")
    else:
        logger.info(f"📊 {total}/10 slots passing")
    logger.info("=" * 80)
